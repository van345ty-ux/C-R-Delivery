-- ENSAIO: cria e verifica a fundacao dentro da transacao; desfaz tudo ao final.
BEGIN;
...
CREATE POLICY "Admins manage after hours config" ON public.after_hours_access_config
  FOR ALL TO authenticated USING(public.get_my_role()='admin') WITH CHECK(public.get_my_role()='admin');
CREATE POLICY "Admins manage after hours members" ON public.after_hours_access_members
  FOR ALL TO authenticated USING(public.get_my_role()='admin') WITH CHECK(public.get_my_role()='admin');
...
DECLARE v_user_id uuid:=auth.uid(); v_user_ids uuid[]; v_invalid_count integer;
BEGIN
  IF v_user_id IS NULL OR public.get_my_role() IS DISTINCT FROM 'admin' THEN
    RAISE EXCEPTION 'Administrator access is required' USING ERRCODE='42501'; END IF;
  IF p_mode IS NULL OR p_mode NOT IN ('none','selected','all') THEN
    RAISE EXCEPTION 'Invalid after-hours access mode' USING ERRCODE='22023'; END IF;
  ...
  IF p_mode='selected' AND cardinality(v_user_ids)=0 THEN
    RAISE EXCEPTION 'Select at least one customer' USING ERRCODE='22023'; END IF;
  ...
  IF v_invalid_count>0 THEN RAISE EXCEPTION 'The selection contains an invalid customer' USING ERRCODE='22023'; END IF;
  INSERT INTO public.after_hours_access_config(singleton,mode,updated_at,updated_by)
    ...
    ON CONFLICT(singleton) DO UPDATE SET mode=excluded.mode,updated_at=excluded.updated_at,updated_by=excluded.updated_by;
  DELETE FROM public.after_hours_access_members WHERE user_id IS NOT NULL;
  IF p_mode='selected' THEN INSERT INTO public.after_hours_access_members(user_id,created_by)
    SELECT id,v_user_id FROM unnest(v_user_ids) ids(id); END IF;
  RETURN jsonb_build_object('mode',p_mode,'selected_count',CASE WHEN p_mode='selected' THEN cardinality(v_user_ids) ELSE 0 END);
END;
$function$;
...
DECLARE v_user_id uuid:=auth.uid(); v_mode text;
BEGIN
...
BEGIN
  IF (SELECT mode FROM public.after_hours_access_config WHERE singleton) IS DISTINCT FROM 'none'
    ...
    RAISE EXCEPTION 'O estado inicial seguro nao foi preservado.'; END IF;
  IF NOT EXISTS(SELECT 1 FROM pg_class WHERE oid='public.after_hours_access_config'::regclass AND relrowsecurity)
     OR NOT EXISTS(SELECT 1 FROM pg_class WHERE oid='public.after_hours_access_members'::regclass AND relrowsecurity) THEN
    RAISE EXCEPTION 'RLS nao foi ativado.'; END IF;
  IF has_table_privilege('anon','public.after_hours_access_config','SELECT')
     OR has_table_privilege('anon','public.after_hours_access_members','SELECT')
     OR has_function_privilege('anon','public.get_my_after_hours_access()','EXECUTE') THEN
    RAISE EXCEPTION 'Visitantes receberam acesso indevido.'; END IF;
  IF NOT has_function_privilege('authenticated','public.get_my_after_hours_access()','EXECUTE')
     OR NOT has_function_privilege('authenticated','public.set_after_hours_access(text,uuid[])','EXECUTE') THEN
    RAISE EXCEPTION 'As funcoes autenticadas nao ficaram disponiveis.'; END IF;
END;
$$;

ROLLBACK;
SELECT 'Ensaio da fundacao de acesso fora do horario concluido. Todas as alteracoes desta transacao foram desfeitas.' AS resultado;
