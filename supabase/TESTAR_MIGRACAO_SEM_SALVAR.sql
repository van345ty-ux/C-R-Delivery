-- ENSAIO NO BANCO: executa a migração dentro de uma transação e desfaz no final.
-- Não ativa a proteção, não cria pedidos e não envia notificações.
-- Executar o arquivo inteiro, incluindo ROLLBACK. Não executar apenas trechos.
-- Pode aguardar brevemente locks de tabelas; interrompe se o limite for excedido.
...
SELECT 'Ensaio concluído. Todas as alterações desta transação foram desfeitas.' AS resultado;
