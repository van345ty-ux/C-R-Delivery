  ...
  try {
    ...
    if (requestPayload.project_type) {
      ...
      if (project_type === 'delivery') {
        targetWebhookUrl = Deno.env.get('N8N_DELIVERY_WEBHOOK_URL') || 'https://n8n.meuapp-on.online/webhook/whatsapp-order-notification-V2';
      } else if (project_type === 'marketing') {
        targetWebhookUrl = Deno.env.get('N8N_MARKETING_WEBHOOK_URL') || Deno.env.get('MARKETING_WEBHOOK_URL') || 'https://n8n.meuapp-on.online/webhook/iniciar-campanha';
      } else if (project_type === 'nail-designer' || project_type === 'nail_scheduler') {
        targetWebhookUrl = Deno.env.get('N8N_NAIL_DESIGNER_WEBHOOK_URL') || Deno.env.get('N8N_NAIL_WEBHOOK_URL') || 'https://n8n.meuapp-on.online/webhook/nail-designer';
      } else {
    ...
    // Lê a nova Secret. Se não achar, usa a URL como fallback (como garantia).
    const pixWebhookUrl = Deno.env.get('N8N_PIX_WEBHOOK_URL') || 'https://n8n.meuapp-on.online/webhook/pix-academias';
