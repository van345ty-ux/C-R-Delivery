...
export const ProductCard: React.FC<ProductCardProps> = ({ product, onAddToCart, isPromotion = false }) => {
  return (
    <div className={cn(
      ...
      isPromotion ? "p-1 bg-red-50 border-red-300 shadow-md" : "p-1 space-y-1 gap-0 bg-white border-gray-200" // Ajustado padding para p-2
    )}>
      {/* Imagem e informações básicas no topo */}
      <div className={cn(
        "flex items-center",
        isPromotion ? "gap-2 mb-1" : "gap-1 mb-0" // Ajustado gap para o card normal
      )}>
        <img 
          src={product.image} 
          alt={product.name} 
          className={cn(
            ...
            isPromotion ? "w-[54%] h-36" : "w-[36%] h-[100%] " // Reduzido o tamanho da imagem para o card normal
          )} 
        />
