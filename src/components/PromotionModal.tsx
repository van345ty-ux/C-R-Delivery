        ...
        <div className="p-6 overflow-y-auto space-y-4">
          {promotions.map((promo) => (
            <div key={promo.id} className="flex items-center gap-4 bg-gray-50 p-4 rounded-lg">
              <img src={promo.image} alt={promo.name} className="p-1 rounded-md hover:scale-105 transition-transform duration-300">
  <img
    src={promo.image}
    alt={promo.name}
    className="w-40 h-28 object-cover object-left rounded-md flex-shrink-0"
  />
